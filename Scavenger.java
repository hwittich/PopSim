...
public class Scavenger extends Organism {
	public Scavenger(Species species) {
		setSpeciesName(species);
		this.getSpeciesName().grow(this);
		setIsAlive(true);
		setOrganismID();
		setImage(new ImageIcon(new ImageIcon("scavengers.png").getImage().getScaledInstance(40, 40, Image.SCALE_DEFAULT)));
		setEnergy(0.5);
		setNutrients(100);
		setSpeed(3);
		setPerception(3);
	...
	public Scavenger(Species species, int speed, int perception, int camouflage, int strength) {
				...
				for (int row = xLowSearchDistance; row <= xHighSearchDistance; row++) {
					if (this.getSpeciesName().getMap().getAcre(row, column).getOrganisms().size() > 0) {
						for (int organism = 0; organism < this.getSpeciesName().getMap().getAcre(row, column).getOrganisms().size(); organism++) {
							if ((this.getSpeciesName().getMap().getAcre(row, column).getOrganisms().get(organism) instanceof Herbivore || this.getSpeciesName().getMap().getAcre(row, column).getOrganisms().get(organism) instanceof Predator || this.getSpeciesName().getMap().getAcre(row, column).getOrganisms().get(organism) instanceof Scavenger) && !(this.getSpeciesName().getMap().getAcre(row, column).getOrganisms().get(organism) == this)) {
								distance = 0;
								...
								if (this.getSpeciesName().getMap().getAcre(row, column).getOrganisms().get(organism).getIsAlive() == false && (this.getSpeciesName().getMap().getAcre(row, column).getOrganisms().get(organism) instanceof Herbivore || this.getSpeciesName().getMap().getAcre(row, column).getOrganisms().get(organism) instanceof Predator || this.getSpeciesName().getMap().getAcre(row, column).getOrganisms().get(organism) instanceof Scavenger) && distance <= this.getPerception() - this.getSpeciesName().getMap().getAcre(row, column).getOrganisms().get(organism).getCamouflage()) {
								...
								if (this.getSpeciesName().getMap().getAcre(row, column).getOrganisms().get(organism).getIsAlive() == true && this.getSpeciesName().getMap().getAcre(row, column).getOrganisms().get(organism) instanceof Predator && distance <= this.getPerception() - this.getSpeciesName().getMap().getAcre(row, column).getOrganisms().get(organism).getCamouflage()) {
	...
	public void eat() {
		if (this.getIsAlive() == true) {
			if (this.getSpeciesName().getMap().getAcre(this.getX(), this.getY()).getOrganisms().size() > 1) {
				int organismNutrients = 0;
				Organism nutritiousOrganism = null;
				for (int organism = 0; organism < this.getSpeciesName().getMap().getAcre(this.getX(), this.getY()).getOrganisms().size(); organism++) {
					if ((this.getSpeciesName().getMap().getAcre(this.getX(), this.getY()).getOrganisms().get(organism) instanceof Herbivore || this.getSpeciesName().getMap().getAcre(this.getX(), this.getY()).getOrganisms().get(organism) instanceof Predator || this.getSpeciesName().getMap().getAcre(this.getX(), this.getY()).getOrganisms().get(organism) instanceof Scavenger) && this.getSpeciesName().getMap().getAcre(this.getX(), this.getY()).getOrganisms().get(organism).getIsAlive() == false && !(this.getSpeciesName().getMap().getAcre(this.getX(), this.getY()).getOrganisms().get(organism) == this)) {
						if (this.getSpeciesName().getMap().getAcre(this.getX(), this.getY()).getOrganisms().get(organism).getNutrients() > organismNutrients) {
							organismNutrients = this.getSpeciesName().getMap().getAcre(this.getX(), this.getY()).getOrganisms().get(organism).getNutrients();
							nutritiousOrganism = this.getSpeciesName().getMap().getAcre(this.getX(), this.getY()).getOrganisms().get(organism);
						}
					}
				}
				
				if (!(nutritiousOrganism == null)) {
					this.setNutrients(this.getNutrients() + (int)(nutritiousOrganism.getNutrients() * (0.1 * this.getStrength())));
					nutritiousOrganism.setNutrients((int)(nutritiousOrganism.getNutrients() * (1 - (0.1 * this.getStrength()))));
				}
			}
		}
	}
		...
		if (this.getIsAlive() == true) {
			if (this.getEnergy() > this.getEnergyToReproduce()) {
				for (int i = 0; i < this.getNumOffspring(); i++) {
					...
					Organism child = new Scavenger(this.getSpeciesName());
