...
public class Herbivore extends Organism {
	public Herbivore(Species species) {
		...
		setIsAlive(true);
		setOrganismID();
		setImage(new ImageIcon(new ImageIcon("herbivores.png").getImage().getScaledInstance(40, 40, Image.SCALE_DEFAULT)));
		setEnergy(0.5);
		setNutrients(100);
		setSpeed(3);
		setPerception(3);
	...
	public Herbivore(Species species, int speed, int perception, int camouflage, int strength) {
		...
		setNumberOfOffspring(2);
		setEnergyToReproduce(0.6);
	}
	
	//search through acres within perception range
	//calculate distance from target organism (change in y + change in x)
	//if the distance is less than or equal to the host organism's perception minus the target organism's camouflage, then it is stored as the nearest organism and the distance is stored
	//if another organism is found with a lower distance, this new organism is stored.
	//search the area again for predators
	//whichever is closer, move towards it
	//After searching for all organisms, host randomly moves by x or y towards the target organism (or only by x if in same y or only by y if in same x)
	//if predator and prey are equidistant, randomly move towards one or away from the other
	public void move() {
				...
				for (int row = xLowSearchDistance; row <= xHighSearchDistance; row++) {
					if (this.getSpeciesName().getMap().getAcre(row, column).getOrganisms().size() > 0) {
						for (int organism = 0; organism < this.getSpeciesName().getMap().getAcre(row, column).getOrganisms().size(); organism++) {
							if (this.getSpeciesName().getMap().getAcre(row, column).getOrganisms().get(organism).getIsAlive() == true && (this.getSpeciesName().getMap().getAcre(row, column).getOrganisms().get(organism) instanceof Plant || this.getSpeciesName().getMap().getAcre(row, column).getOrganisms().get(organism) instanceof Predator) && !(this.getSpeciesName().getMap().getAcre(row, column).getOrganisms().get(organism) == this)) {
								distance = 0;
								...
								if (this.getSpeciesName().getMap().getAcre(row, column).getOrganisms().get(organism) instanceof Plant && distance <= this.getPerception() - this.getSpeciesName().getMap().getAcre(row, column).getOrganisms().get(organism).getCamouflage()) {
	...
	public void eat() {
		if (this.getIsAlive() == true) {
			if (this.getSpeciesName().getMap().getAcre(this.getX(), this.getY()).getOrganisms().size() > 1) {
				int plantStrength = 2147483647;
				Organism weakestPlant = null;
				for (int organism = 0; organism < this.getSpeciesName().getMap().getAcre(this.getX(), this.getY()).getOrganisms().size(); organism++) {
					if (this.getSpeciesName().getMap().getAcre(this.getX(), this.getY()).getOrganisms().get(organism) instanceof Plant && this.getSpeciesName().getMap().getAcre(this.getX(), this.getY()).getOrganisms().get(organism).getIsAlive() == true) {
						if (this.getSpeciesName().getMap().getAcre(this.getX(), this.getY()).getOrganisms().get(organism).getStrength() < plantStrength) {
							plantStrength = this.getSpeciesName().getMap().getAcre(this.getX(), this.getY()).getOrganisms().get(organism).getStrength();
							weakestPlant = this.getSpeciesName().getMap().getAcre(this.getX(), this.getY()).getOrganisms().get(organism);
						}
					}
				}
				if (!(weakestPlant == null)) {
					if (this.getStrength() > weakestPlant.getStrength()) {
						if (weakestPlant.getEnergy() > (0.1 * (this.getStrength() - weakestPlant.getStrength()))) {
							weakestPlant.setEnergy(weakestPlant.getEnergy() - (0.1 * (this.getStrength() - weakestPlant.getStrength())));
							this.setNutrients(this.getNutrients() + (int)(weakestPlant.getNutrients() * (0.1 * (this.getStrength() - weakestPlant.getStrength()))));
							weakestPlant.setNutrients((int)(weakestPlant.getNutrients() * (1 - (0.1 * (this.getStrength() - weakestPlant.getStrength())))));
						} else {
							weakestPlant.setEnergy(0);
							this.setNutrients(this.getNutrients() + (int)(weakestPlant.getNutrients() * (0.1 * this.getStrength())));
							weakestPlant.setNutrients((int)(weakestPlant.getNutrients() * (1 - (0.1 * this.getStrength()))));
						}
					} else if (this.getStrength() < weakestPlant.getStrength()) {
						if (this.getEnergy() > (0.1 * (weakestPlant.getStrength() - this.getStrength()))) {
							this.setEnergy(this.getEnergy() - (0.1 * (weakestPlant.getStrength() - this.getStrength())));
							this.setNutrients(this.getNutrients() + (int)(weakestPlant.getNutrients() * (0.1 / (weakestPlant.getStrength() - this.getStrength()))));
							weakestPlant.setNutrients((int)(weakestPlant.getNutrients() * (1 - (0.1 / (weakestPlant.getStrength() - this.getStrength())))));
						} else {
							this.setEnergy(0);
						}
					} else {
						this.setNutrients(this.getNutrients() + (int)(weakestPlant.getNutrients() * 0.1));
						weakestPlant.setNutrients((int)(weakestPlant.getNutrients() * 0.9));
					}
				}
			}
		}
		...
		if (this.getIsAlive() == true) {
			if (this.getEnergy() > this.getEnergyToReproduce()) {
				for (int i = 0; i < this.getNumOffspring(); i++) {
					...
					Organism child = new Herbivore(this.getSpeciesName());
